...
export const Route = createFileRoute("/trainer/reviews/$id")({
  head: () => seo("Review", "Review detail and reply."),
...
function ReviewDetail() {
  ...
  if (!r) return <EmptyState title="Review not found" action={<Button asChild><Link to="/trainer/reviews">Back</Link></Button>} />;
  return (
    <>
      <Link to="/trainer/reviews" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> My reviews</Link>
        ...
        <div className="mt-6 border-t border-border pt-5">
          ...
          {editing ? (
            <div className="mt-3 space-y-3">
              <Textarea rows={4} value={text} onChange={(e) => setText(e.target.value)} placeholder="Thank the client, address any concerns…" maxLength={400} />
              <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">{text.length}/400</span>
                <Button disabled={text.trim().length < 5} onClick={() => { reviewStore.upsert({ ...r, reply: text }); setEditing(false); toast.success("Reply published"); }}>Publish reply</Button>
              </div>
            </div>
          ) : (
            <div className="mt-3 rounded-md bg-muted p-4 text-sm">
              <p className="text-xs font-medium text-primary">{trainerMe.name} replied</p>
              <p className="mt-1">{r.reply}</p>
              <Button size="sm" variant="ghost" className="mt-2" onClick={() => setEditing(true)}>Edit reply</Button>
            </div>
          )}
        </div>
      </article>
